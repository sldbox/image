...
- 태블릿 등 좁은 가로 폭에서 텍스트 깨짐을 방지하는 flex-wrap 최적화 적용
=============================================================================
*/
...
const I18N = {
    creator: {ko:'제작자', en:'Creator'}, creator_name: {ko:'회장', en:'Hoe-jang'},
...
const dashboardAtoms = ["Warmonger", "Spartan Company", "Dark Zealot", "Dark Sentry", "Primal Roach", "Sniper", "Diamondback", "Dark High Templar", "Dark Stalker", "Primal Lurker", "Hammer Securities", "Siege Breaker", "Dark Archon", "Dark Immortal", "Primal Hydralisk", "Widow Mine", "Auto Turret", "Yeti(F)", "Yeti(M)", "GOT/Massive"];

const specialKeywordMap = [
    {keys:["자동포탑", "auto turret"], atom:"Auto Turret"}, {keys:["잠복", "burrow"], atom:"Burrow"},
    {keys:["지뢰", "시체매", "widow mine", "spider mine", "vulture-mineplant"], atom:"Widow Mine", divider:12},
    {keys:["우르사돈[암]", "우르사돈암", "yeti(f)", "yetif"], atom:"Yeti(F)"}, {keys:["우르사돈[수]", "우르사돈수", "yeti(m)", "yetim"], atom:"Yeti(M)"},
    {keys:["메시브", "디제스터", "massive", "disaster"], atom:"GOT/Massive", subKey:"Massive"}, {keys:["갓오브타임", "갓오타", "god of time", "got"], atom:"GOT/Massive", subKey:"GOT"}
];
...
function processCostKeyword(rawName,amount,map){
    ...
    for(const conf of specialKeywordMap){
        if(conf.keys.some(k=>cName.includes(clean(k)))){
            ...
            if(conf.subKey){ if(!map[conf.atom])map[conf.atom]={GOT:0,Massive:0}; map[conf.atom][conf.subKey]+=val; }
...
function toggleSelectAllInCurrentTab() {
    ...
    const items = Array.from(new Set(Array.from(unitMap.values()))).filter(u => u.category === catKey && ["SuperHidden","Hidden","Legend"].includes(u.grade));
...
function updateMagicDashboard(){
    ...
    const totalMap={}; dashboardAtoms.forEach(a=>{if(a==="GOT/Massive")totalMap[a]={GOT:0,Massive:0};else totalMap[clean(a)]=0;});
    ...
    dashboardAtoms.forEach(a=>{
        const key = a==="GOT/Massive" ? a : clean(a);
        const val=totalMap[key], container=document.getElementById(`vslot-${clean(a)}`);if(!container)return;
...
// === [수정완료] 가로폭 부족 시 레시피 텍스트가 찌그러지는 오류 방지 (flex-wrap 속성 적용) ===
function formatRecipeVertical(item) {
    ...
    item.recipe.ko.split(/\+(?![^()]*\))/).forEach((part, index) => {
        ...
        if (match) {
            ...
            // flex-wrap: wrap;을 적용하여 좁아질 때 span이 아래로 떨어지도록 안정성 확보
...
function renderCurrentTabContent() {
    const catKey = TAB_CATEGORIES[_activeTabIdx].key;
    let items = Array.from(new Set(Array.from(unitMap.values()))).filter(u => u.category === catKey && ["SuperHidden","Hidden","Legend"].includes(u.grade));
    ...
    items.forEach(item => {
        ...
        // [수정완료] 우측 컨트롤 패널 flex 속성 최적화 (flex: 1 1 auto)
        let rightControls = `<div style="flex:1 1 auto; padding:10px; display:flex; gap:8px; align-items:center; justify-content:flex-end;" onclick="event.stopPropagation()">${essenceBox}`;
        ...
        // [수정완료] 유닛 카드의 flex-basis 구조 최적화 
        h+=`<div class="unit-card ${isActive?'active':''}" style="display:flex; flex-direction:column; padding:0; background:${cardBg}; border:1px solid ${cardBorder}; border-left:3px solid ${isActive?'var(--g)':'transparent'}; border-radius:3px; min-height:85px; margin-bottom:4px; cursor:pointer; transition:all 0.2s;" onclick="toggleUnitSelection('${item.id}')">
            <div style="display:flex; align-items:stretch; width:100%; flex-wrap:wrap;">
                
                <div style="flex:1 1 160px; max-width:220px; padding:10px 12px; display:flex; align-items:center;">
                    <div style="color:${gradeColorsRaw[item.grade]}; font-weight:bold; font-size:1.1rem; display:flex; align-items:center; gap:8px;">
                    <span class="gtag" style="color:${gradeColorsRaw[item.grade]};border-color:${gradeColorsRaw[item.grade]}44;">${GRADE_SHORT[item.grade]}</span> ${nameDisp}</div>
                </div>
                <div style="width:1px; background:var(--border); margin:10px 0;"></div>
                
                <div style="flex:3 1 200px; padding:10px 12px; display:flex; flex-direction:column; justify-content:center;">${formatRecipeVertical(item)}</div>
                <div style="width:1px; background:var(--border); margin:10px 0;"></div>
...
function resetMagicDashboard(){
    ...
    dashboardAtoms.forEach(a=>{
        const u=unitMap.get(clean(a)); const el = document.getElementById(`name-${clean(a)}`);
        if(el) { el.innerHTML = u ? getDisplayText(u.name.ko, u.name.en) : a; }
    });
}
