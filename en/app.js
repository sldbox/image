...
function renderJewelGrid(){
    ...
    for(let i=0;i<JEWEL_RAW_DATA.length;i+=4){
        ...
        h+=`<div onclick="openJewelPopup('${en}','${leg}','${myth}','${c}')" style="cursor:pointer;display:flex;flex-direction:column;align-items:center;background:var(--jewel-card);padding:15px;border-radius:10px;border:1px solid var(--border);"><div style="width:120px;height:120px;border-radius:50%;border:2px solid ${c}80;display:flex;justify-content:center;align-items:center;margin-bottom:15px;overflow:hidden;background:var(--jewel-card);"><img src="${url}${en}.png" style="width:145%;height:145%;object-fit:cover;"></div><div style="font-size:2rem; font-weight:bold; margin-bottom:20px; color:var(--text); font-family:var(--font-display);">${en.toUpperCase()}</div></div>`;
    }
    g.innerHTML=h;
}
